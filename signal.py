...
def signal(symbol ,data ,databig , position , signal, sleep , file ,initialsignal):
	...
	if histogram > 0 and position[define.position] != "LONG":
		...
		file.write('\n')
		sleep = 0
		if abs(data[define.lastramp]-data[define.ramp])/data[define.price] /3000 * 1000000 < 0.09 or databig[define.biglasthistogram]  > databig[define.bighistogram] or initialsignal == 1:
			sleep =1
